...
- Focused tests were attempted with `npm --workspace @franken/orchestrator run test`.
- Typecheck/tests were blocked in this environment because package dependencies and node typings are not available in the workspace (`Cannot find module '...'/missing `@types/node` and workspace packages).
...
- `.env.test` is intentionally ignored and documented via `.env.test.example` + `.gitignore`.
